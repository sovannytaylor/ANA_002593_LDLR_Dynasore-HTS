...
def read_fcs_csv(file_path: Path) -> pd.DataFrame:
    ...
    return pd.read_csv(file_path, skiprows=0)


def extract_well_id(file_path: Path) -> str:
    """Extract WELL ID from the FlowJo CSV filename."""
    name = file_path.stem  # no extension
    # Find something like A01, B12, C03 etc
